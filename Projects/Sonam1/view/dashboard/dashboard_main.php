...
include "get_business_rules.php";
...
<input type="hidden" id="reminder_status" name="reminder_status" value="<?= $reminder_status ?>">
      <input type="hidden" id='cache_rules' value='<?= $new_array ?>'/>
<?php
if($role=="Admin"){
...
(function(){
	...
	var cache_rules = JSON.parse($('#cache_rules').val());
	//console.log(cache_rules);
	// var test = JSON.parse(cache_rules['tax_rules'][0]['conditions']);
	// console.log(test[0]['for1']);
	var status = false;
