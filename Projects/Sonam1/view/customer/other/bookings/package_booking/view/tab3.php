	                    ...
	                    <tbody>
	                       <?php 
	                       		...
	                       		while($row_entry = mysql_fetch_assoc($sq_entry)){
	                       			...
	                       			$sq_bus_name = mysql_fetch_assoc(mysql_query("select * from transport_agency_bus_master where bus_id='$transport_bus_id'"));
	                       			$count++;
	                       	?>
							<tr class="<?php echo $bg; ?>">
							    <td><?php echo $sq_bus_name['bus_name'] ?></td>
								<td><?php echo get_date_user($row_entry['transport_from_date']); ?></td>
							    <td><?php echo get_date_user($row_entry['transport_to_date']); ?></td>
							</tr>  
							<script>
								$("#birth_date<?= $offset.$count ?>_d, #expiry_date<?= $offset ?>1").datetimepicker({ timepicker:false, format:'d-m-Y' });
							</script>      
