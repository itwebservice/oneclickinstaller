...
$branch_status = $_POST['branch_status'];
		...
		while($row_visa_payment = mysql_fetch_assoc($sq_visa_payment)){
			...
			$sq_pay = mysql_fetch_assoc(mysql_query("select sum(payment_amount) as sum from miscellaneous_payment_master where clearance_status!='Cancelled' and misc_id='$row_visa_payment[misc_id]'"));
			...
			$temp_arr = array( "data" => array(
				...
				$payshow,
				$row_visa_payment['payment_amount'],
				'<a onclick="loadOtherPage(\''. $url1 .'\')" class="btn btn-info btn-sm" title="Download Receipt"><i class="fa fa-print"></i></a>
	
				<button data-toggle="tooltip" class="btn btn-info btn-sm" onclick="exc_payment_update_modal('. $row_visa_payment['payment_id'] .')" title="Update Details"><i class="fa fa-pencil-square-o"></i></button>'
			  ), "bg" =>$bg );
