...
function get_enquiry_details(offset)
{
	...
	$.ajax({
		...
		success:function(result){
			...
			$('#service_tax'+offset).val(result[0].service_tax);
			...
			$('#driver_allowance'+offset).val(result[0].driver_allowance);
			$('#total_tour_cost'+offset).val(result[0].total_tour_cost);
			...
			$('#markup_cost_subtotal'+offset).val(result[0].markup_cost_subtotal);
			$('#state_entry_tax'+offset).val(result[0].state_entry);
			$('#other_charges'+offset).val(result[0].other_charges);
			$('#total_hrs'+offset).val(result[0].total_hrs);
			$('#total_km'+offset).val(result[0].total_km);
			$('#traveling_date'+offset).val(result[0].total_km);
...
function calculate_total_fees(offset="")
{
	...
	if(parseFloat(markup_cost) == 0){
   		var t_subtotal = parseFloat(basic_amount) + parseFloat(markup_cost_subtotal);
    }
    else{
   	   var markup_cost_subtotal = (parseFloat(basic_amount)/100) * parseFloat(markup_cost);
	   var t_subtotal = parseFloat(basic_amount) + parseFloat(markup_cost_subtotal);
    }
	if(markup_cost_subtotal==""){markup_cost_subtotal = 0;}
	var service_tax_cost = (parseFloat(t_subtotal)/100)*parseFloat(service_tax);
	service_tax_cost = Math.round(service_tax_cost);
	$('#markup_cost_subtotal'+offset).val(markup_cost_subtotal.toFixed(2));
	$('#service_tax_subtotal'+offset).val(service_tax_cost.toFixed(2));
	var total_cost = ( parseFloat(basic_amount) +parseFloat(markup_cost_subtotal)+ parseFloat(service_tax_cost) );
