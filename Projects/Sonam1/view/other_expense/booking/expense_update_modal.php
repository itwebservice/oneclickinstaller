    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <div class="row">
            ...
            <div class="col-md-4 col-sm-6 col-xs-12 mg_bt_10">
              <select name="taxation_type1" id="taxation_type1" title="Taxation Type" value="<?= $q_expense['taxation_type'] ?>" style="width: 100%" class="form-control">
              <option value="<?= $q_expense['taxation_type'] ?>"><?= $q_expense['taxation_type'] ?></option>
                <?php get_taxation_type_dropdown($setup_country_id) ?>
              </select>
            </div>
            <div class="col-md-4 col-sm-6 col-xs-12 mg_bt_10">
              <select name="taxation_id1" id="taxation_id1" title="Tax" value="<?= $q_expense['taxation_id'] ?>" onchange="generic_tax_reflect(this.id, 'service_tax1', 'total_fun_update');" style="width: 100%" class="form-control">                   
                   <?php 
                   if($q_expense['taxation_id']!="0"){

                     $sq_taxation = mysql_fetch_assoc(mysql_query("select * from taxation_master where taxation_id='$q_expense[taxation_id]'"));
              ...
              <input type="hidden" id="service_tax1" name="service_tax1" value="<?= $q_expense['service_tax'] ?>">
             </div>                        
            <div class="col-md-4 col-sm-6 col-xs-12 mg_bt_10">
                <input type="text" id="service_tax_subtotal1" name="service_tax_subtotal1" placeholder="Tax Amount" value="<?= $q_expense['service_tax_subtotal'] ?>" title="Tax Amount" style="width: 100%" class="form-control" readonly>
            </div>          
            <div class="col-md-4 col-sm-6 col-xs-12 mg_bt_10">
...
{ 
    ...
    var service_tax_subtotal = (parseFloat(service_tax)/100)*parseFloat(sub_total);
          ...
          if(data !== 'valid'){
            ...
            return false;
	            ...
	            $.ajax({
	              ...
	              data:{ expense_id : expense_id,expense_type : expense_type, supplier_type : supplier_type, sub_total : sub_total, taxation_type : taxation_type, service_tax : service_tax,taxation_id : taxation_id, service_tax_subtotal : service_tax_subtotal, tds : tds, net_total : net_total, due_date : due_date, booking_date : booking_date, invoice_no : invoice_no, id_upload_url : id_upload_url },
