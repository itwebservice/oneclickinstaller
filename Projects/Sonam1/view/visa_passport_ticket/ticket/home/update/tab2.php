...
$("#"+id1).autocomplete({
	...
	change: function(event,ui){
		...
		if(!ui.item) {
			 ...
			 error_msg_alert('Please select From Airport from the list!!'); 
			 }
	}
...
$("#"+id2).autocomplete({
	...
	change: function(event,ui){
		...
		if(!ui.item) {
			 ...
			 error_msg_alert('Please select To Airport from the list!!');
			 }
	}
}).data("ui-autocomplete")._renderItem = function(ul, item) {
      return $("<li disabled>")
...
function copy_values(){
	...
	$('#flight_no-'+count).val($('#flight_no-1').val());
	$('#airlin_pnr-'+count).val($('#airlin_pnr-1').val());
