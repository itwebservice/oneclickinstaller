...
<div class="row mg_bt_10">
	...
	<div class="col-md-4">
		...
		<select id="sector_from" name="sector_from" class="app_select2" style="width:100%" title="Sector From" class="form-control">
			...
			<?php $sq_airport = mysql_query("select airport_id,airport_name, airport_code from airport_master where flag!='Inactive' order by airport_name asc");
    		...
    		<?php
				...
				$f_airname = mysql_fetch_assoc(mysql_query("select airport_code,airport_name from airport_master where airport_id = ".$sq_c['sector_from']));
			?>

			<option  value="<?= $sq_c['sector_from'] ?>"><?php echo $f_airname['airport_name']." (".$f_airname['airport_code'].")"; ?></option>
	...
	<div class="col-md-4">
		...
		<select id="sector_to" name="sector_to" class="app_select2" style="width:100%" title="Sector To" class="form-control">
			...
			<?php }  }
				$t_airname = mysql_fetch_assoc(mysql_query("select airport_name,airport_code from airport_master where airport_id = ".$sq_c['sector_to']));
			?>

			<option value="<?= $sq_c['sector_to']?>"><?php echo $t_airname['airport_name']." (".$t_airname['airport_code'].")"; ?></option>
...
<div class="row mg_bt_10">
	...
	<div class="col-md-4">
		<select id="preffered_airline" name="preffered_airline" class="app_select2" style="width:100%" title="Preferred Airline" class="form-control">
			...
			<option value="">*Airline</option>
					<?php  $sq_airline = mysql_query("select * from airline_master where active_flag!='Inactive' order by airline_name asc");
		while($row_airline = mysql_fetch_assoc($sq_airline)){
			?>
			<option value="<?= $row_airline['airline_id'] ?>"><?= $row_airline['airline_name'].' ('.$row_airline['airline_code'].')' ?></option>
			<?php
