...
public function expense_save()
{
	...
	$sq_expense = mysql_query("insert into other_expense_master (expense_id, expense_type_id, supplier_id, financial_year_id, branch_admin_id, amount,taxation_type, taxation_id, service_tax, service_tax_subtotal, tds, total_fee, due_date,invoice_no,expense_date,invoice_url,created_at) values ('$expense_id', '$expense_type', '$supplier_type', '$financial_year_id', '$branch_admin_id', '$sub_total','$taxation_type' ,'$taxation_id','$service_tax','$service_tax_subtotal', '$tds','$net_total','$due_date','$invoice_no','$booking_date','$id_upload_url','$created_at' ) ");
...
public function finance_save($expense_id, $payment_id, $row_spec, $branch_admin_id){
    ...
    /////////Tax Amount/////////
	purchase_tax_reflection_update('Other Expense Booking',$service_tax_subtotal,$taxation_type,$expense_id,get_other_expense_booking_id($expense_id),$booking_date,$payment_mode, $row_spec);
    ...
    if($supplier_type != ''){
	    ...
	    $payment_date = $booking_date;
	    $payment_particular = get_expense_paid_particular(get_other_expense_booking_id($expense_id), $expense_type, $booking_date, $balance_amount, $payment_mode);
	    $ledger_particular = get_ledger_particular('To','Expense');
	    $gl_id = $cust_gl;
	    $payment_side = "Credit";
	    $clearance_status = "";
...
public function expense_update(){
	...
	begin_t();
	$sq_expense_u = mysql_query("update other_expense_master set expense_type_id='$expense_type',supplier_id='$supplier_type',amount='$sub_total',taxation_type='$taxation_type', taxation_id='$taxation_id', service_tax='$service_tax', service_tax_subtotal='$service_tax_subtotal', tds='$tds', total_fee='$net_total', due_date='$due_date',invoice_no='$invoice_no',expense_date='$booking_date',invoice_url='$id_upload_url' where expense_id='$expense_id'");
