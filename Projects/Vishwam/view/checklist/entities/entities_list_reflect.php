		...
		<tbody>
			...
			while($row_entity = mysql_fetch_assoc($sq_entity)){
				...
				<tr>
					...
					<td><?= ($sq_tour['tour_name']=="")? "NA" : $sq_tour['tour_name'] ?></td>
					<td><?= ($row_entity['tour_group_id']=="") ? "NA" : get_date_user($sq_tour_group['from_date']).' To '.get_date_user($sq_tour_group['to_date']) ?></td>
					<td><?= ($row_entity['booking_id']=="") ? "NA" : get_package_booking_id($row_entity['booking_id']) ?></td>
