...
$objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('K'.$row_count, "Travel Amount") 
        ->setCellValue('L'.$row_count, "Tax Amount")
        ->setCellValue('M'.$row_count, "Visa Amount")
        ->setCellValue('N'.$row_count, "Insurance Amount")
        ->setCellValue('O'.$row_count, "Total Tour Amount")
        ->setCellValue('P'.$row_count, "Total Amount")
        ->setCellValue('Q'.$row_count, "Paid Amount")
        ->setCellValue('R'.$row_count, "Balance Amount")
        ->setCellValue('S'.$row_count, "Due Date")
        ->setCellValue('T'.$row_count, "Created By");
...
$row_count++;
    ...
    while($row_booking = mysql_fetch_assoc($sq_booking)){
      ...
      //Tour TOtal
      $tour_amount= ($row_booking['total_tour_fee']!="") ? $row_booking['total_tour_fee']: 0;
      ...
      $total_booking_amt = $tour_amount + $total_travel_amount;
      $total_tour_amount = $total_booking_amt - $cancel_amount;

      $sq_paid = mysql_fetch_assoc(mysql_query("select sum(amount) as paid_amount from payment_master where tourwise_traveler_id='$row_booking[id]' and clearance_status !='Pending' and clearance_status !='Cancelled'"));
      $balance_amount = $total_tour_amount - $sq_paid['paid_amount'];
        ...
        ->setCellValue('K'.$row_count, number_format($total_travel_amount,2))
        ->setCellValue('L'.$row_count, number_format($row_booking['service_tax'],2))
        ->setCellValue('M'.$row_count, number_format($visa_total_amount,2))
        ->setCellValue('N'.$row_count, number_format($insuarance_total_amount,2))
        ->setCellValue('O'.$row_count, number_format($tour_amount, 2))
        ->setCellValue('P'.$row_count, number_format($total_tour_amount, 2))
        ->setCellValue('Q'.$row_count, number_format($sq_paid['paid_amount'], 2))
        ->setCellValue('R'.$row_count, number_format($balance_amount, 2))
        ->setCellValue('S'.$row_count, ($row_booking['balance_due_date']=='1970-01-01') ? 'NA' : date('d-m-Y',strtotime($row_booking['balance_due_date'])))
