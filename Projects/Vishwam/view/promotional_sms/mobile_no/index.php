...
<div class="row"> <div class="col-md-12 no-pad"> <div class="table-responsive">
  <table id="tbl_mobile_no" class="table table-hover" style="margin: 20px 0 !important;">
    <thead>
...
function mobile_no_list_reflect(limit, start){
  $('#tbl_mobile_no').show();
  var branch_status = $('#branch_status').val();

	$.ajax({
    url:"mobile_no/mobile_no_list_reflect.php",
    method:"POST",
    data:{branch_status : branch_status,limit:limit, start:start },
    cache:false,
    success:function(data){
      $('#tbl_mobile_no tbody').append(data);
        action = 'inactive';
    }
	});
}
...
if(action == 'inactive'){
	...
	action = 'active';
	mobile_no_list_reflect(limit, start);
}
$("div.app_content_wrap").on('scroll' , function(event){
if($('#tbl_mobile_no').is(":visible")==true){
	if($("div.app_content_wrap").scrollTop() + $("div.app_content_wrap").height() > $("#tbl_mobile_no tbody").height() && action == 'inactive'){
		action = 'active';
		start = start + limit;
		setTimeout(function(){
			mobile_no_list_reflect(limit, start);
...
function mobile_list_reflect(limit , start){
		window.limit=20;
		window.start=0;
		$('#tbl_mobile_no').show();
		$('#tbl_mobile_no tbody tr').remove();
		var branch_status = $('#branch_status').val();
		$.post('mobile_no/list_reflect.php', {limit : limit, start : start, branch_status : branch_status}, function(data){
			$('#tbl_mobile_no tbody').html(data);
		});
}
...
function assign_sms_group()
{
	...
	$.ajax({
	    ...
	    var limit = 20; //The number of records to display per request
			var start = 0; //The starting pointer of the data
			mobile_list_reflect(limit, start);
	    }
	});
}
