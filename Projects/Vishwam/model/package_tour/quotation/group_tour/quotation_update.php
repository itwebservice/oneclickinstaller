...
public function plane_entries_update($quotation_id, $from_city_id_arr, $to_city_id_arr,  $plane_from_location_arr, $plane_to_location_arr, $plane_class_arr,$airline_name_arr, $arraval_arr, $dapart_arr, $plane_id_arr)
{
	for($i=0; $i<sizeof($plane_from_location_arr); $i++){
			...
			if($plane_id_arr[$i]=="")
			{
				...
				$sq_plane = mysql_query("insert into group_tour_quotation_plane_entries ( id, quotation_id, from_city, from_location, to_city, to_location,airline_name, class, arraval_time, dapart_time) values ( '$id', '$quotation_id', '$from_city_id_arr[$i]', '$plane_from_location_arr[$i]', '$to_city_id_arr[$i]', '$plane_to_location_arr[$i]','$airline_name_arr[$i]', '$plane_class_arr[$i]', '$arraval_arr[$i]', '$dapart_arr[$i]' )");
				if(!$sq_plane)
				{
			...
			{
				$sq_update=mysql_query("UPDATE `group_tour_quotation_plane_entries` SET `from_location`='$plane_from_location_arr[$i]',`to_location`='$plane_to_location_arr[$i]',airline_name='$airline_name_arr[$i]',`class`='$plane_class_arr[$i]',`arraval_time`='$arraval_arr[$i]',`dapart_time`='$dapart_arr[$i]' , from_city='$from_city_id_arr[$i]', to_city='$to_city_id_arr[$i]' WHERE `id`='$plane_id_arr[$i]'");
				if(!$sq_update)
				{
