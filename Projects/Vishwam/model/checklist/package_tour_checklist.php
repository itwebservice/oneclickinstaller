...
public function package_tour_checklist_save()
{
	...
	for($i=0; $i<sizeof($entity_id_arr); $i++){
		...
		$sq_checklist = mysql_query("insert into checklist_package_tour( id, branch_admin_id, booking_id, entity_id ) values ( '$id', '$branch_admin_id', '$booking_id', '$entity_id_arr[$i]' )");
		if(!$sq_checklist){
