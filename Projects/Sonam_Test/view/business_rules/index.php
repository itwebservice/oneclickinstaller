...
function values_load(id){
	...
	if(condition === '1'){
		html = '<option value="">Select Value</option></option><option value="Statemate">Statemate</option>';
		...
		html = '<option value="">Select Value</option><option value="Cash">Cash</option><option value="NEFT">NEFT</option><option value="RTGS ">RTGS</option><option value="IMPS">IMPS</option><option value="Online">Online</option><option value="Walet">Walet</option>';
		$("#value"+count).html(html);
	}
		...
		html = '<option value="Sale">Sale</option><option value="Refund">Refund</option><option value="Reissue">Reissue</option>';
		$("#value"+count).html(html);
	}
	...
	else if(condition === "5" || condition === "6" || condition === "7" || condition === "12"){
		$.post('get_condition_values.php', {condition : condition}, function(data){
			$("#value"+count).html(data);
		});
	}
	else{
