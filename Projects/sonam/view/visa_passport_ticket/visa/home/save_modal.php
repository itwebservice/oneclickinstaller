...
<script>
$('#issue_date1, #expiry_date1, #payment_date, #due_date,#balance_date,#appointment1').datetimepicker({ timepicker:false, format:'d-m-Y' });
var date = new Date();
...
function alltable_visa_cost(id){
	...
	for(var i=0; i<rowCount; i++){
    	...
    	if(row.cells[0].childNodes[0].checked == true){  
			...
			var markup = row.cells[16].childNodes[0].value;
			total_amt += parseFloat(amt); total_markup += parseFloat(markup);
        }
	}
	if(isNaN(total_amt)) total_amt = 0;
	if(isNaN(total_markup)) total_markup = 0;
	$('#visa_issue_amount').val(total_amt);
	$('#service_charge').val(total_markup);
}
