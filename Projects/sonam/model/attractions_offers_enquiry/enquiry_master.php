...
function enquiry_master_save(){
  ...
  if($customer_fill){
    $fin_year = mysql_fetch_assoc(mysql_query("select financial_year_id from financial_year where active_flag='Active'"));
    $financial_year_id = $fin_year['financial_year_id'];
    $followup_date =  date("Y-m-d H:i");
    $enquiry_date = date("Y-m-d H:i");
  }
    if($financial_year_id == '' && !$customer_fill){
...
function send_sms_enquiry_master($enquiry_id, $mobile_no,$name){
   ...
   $message = "Dear ".$name.", Thank you for showing your interest in ".$app_name.". Your enquiry number is ".$enquiry_id.". Our Tour Expert will contact you in 24hrs. Please visit the website to know more details : ".$app_website.". Contact : ".$app_contact_no." mail us ".$app_email_id.".";
...
public function enquiry_converted_sms_send($enquiry_id,$name)
{
  ...
  $message = "Dear ".$name.", Thanks for choosing a trip with ".$app_name.". Your relationship manager ".$first_name." ".$last_name." will coordinate with you.";
  global $model;
...
public function enquiry_csv_save()
{
    ...
    if(empty($handle) === false) {
        ...
        if(isset($unprocessedArray) && !empty($unprocessedArray))
        {
          ...
          header("Expires: 0");
            $output = fopen($save, "w");  
            fputcsv($output, array('Customer Name' , 'Contact No' , 'Whatsapp No' , 'Gmail Id', 'Interested Tour' , 'Travel From Date' , 'Travel To Date' , 'Budget' , 'Total Adults' , 'Total Childerns' , 'Total Infant' , 'Total Passangers' , 'Children Without Bed' ,'Children With Bed' , 'Enquiry Date' , 'Followup Date' , 'Reference' , 'Assigned Emp Id' , 'Enquiry Type' , 'Other Specification','Location','Hotel Type'));
    ...
    <tr>
      <td>
        <table style="width:100%">
          <tr>
              <td>
                <a style="font-weight:500;font-size:12px;display:block;color:#ffffff;background:'.$theme_color.';text-decoration:none;padding:5px 10px;border-radius:25px;width: 86px;text-align: center;" href="'.BASE_URL.'model/attractions_offers_enquiry/tour_enquiry.php?l_id='.$login_id.'">Enquiry Details</a>
              </td> 
          </tr>
        </table>
      </td>
