...
function pagination_load(dataset, columns, bg_stat = false,footer_string = false,pg_length = 20,table_id = "tbl_list"){ //1. dataset,2.columns titles,3.if want bg color,4.if want footer,5.manual pagelength change
	var html = "";
	...
	if(footer_string){
		...
		for(var i=0;i<parseInt(dataset_main[dataset_main.length - 1].footer_data['total_footers']);i++){
			html += "<th class=\"text-right "+dataset_main[dataset_main.length - 1].footer_data['class'+i]+" \" colspan='"+dataset_main[dataset_main.length - 1].footer_data['col'+i]+"'>"+dataset_main[dataset_main.length - 1].footer_data['foot'+i]+"</th>";
		}
