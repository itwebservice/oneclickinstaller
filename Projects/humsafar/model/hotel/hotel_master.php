...
public function vendor_csv_save(){
    ...
    if(empty($handle) === false){
        while(($data = fgetcsv($handle, ",")) !== FALSE){
            ...
            if($count>0){
                ...
                if($cwb_from > $cwb_to){
                  ...
                  if(preg_match('/^[0-9]*$/', $city_id) && preg_match('/^[0-9 \s]{6,20}+$/', $mobile) && preg_match('/^[0-9]*$/', $state_id) && preg_match('/^[0-9]*$/', $opening_balance) && !empty($side) && !empty($as_on_date) && ($as_on_date!='1970-01-01') && (strlen($mobile)<=20) && !empty($cwb_from) && !empty($cwb_to) && !empty($cwob_from) && !empty($cwob_to)){
        ...
        if(isset($unprocessedArray) && !empty($unprocessedArray))
        {
          ...
          header("Expires: 0");
          $output = fopen($save, "w");  
          fputcsv($output, array('city_id' , 'Hotel_name' , 'Mobile' , 'landline' , 'Email' , 'Contact_Person' , 'Emergency_Contact_No' ,'Hotel_address', 'state_id' , 'Country' , 'Website' , 'Bank_Name' , 'Account_Name' , 'Account_No' , 'Branch' , 'IFSC_swift_Code' , 'Tax_No', 'PAN_TAN_No' , 'Opening_balance' ,'As of Date', 'Hotel_Type' , 'Balance_Side','Child With Bed From Age','Child With Bed To Age','Child Without Bed From Age','Child Without Bed To Age'));   
...
function hotel_master_save($city_id, $hotel_name, $mobile_no, $landline_no, $email_id, $contact_person_name, $immergency_contact_no, $hotel_address, $country, $website, $opening_balance,$rating_star, $active_flag, $bank_name,$account_name,$account_no,$branch, $ifsc_code, $service_tax_no ,$state,$side,$supp_pan,$hotel_image_path,$as_of_date,$description,$policies,$amenities,$hotel_type,$meal_plan,$cwb_from,$cwb_to,$cwob_from,$cwob_to){
  ...
  $sq = mysql_query("insert into hotel_master ( hotel_id, city_id, hotel_name, mobile_no, landline_no, email_id, contact_person_name, immergency_contact_no, hotel_address, country, website, opening_balance, rating_star,meal_plan,hotel_type, bank_name,account_name, account_no, branch, ifsc_code, service_tax_no,active_flag, state_id,side,pan_no,as_of_date,description,policies,amenities, `cwb_from`, `cwb_to`, `cwob_from`, `cwob_to`) values ( '$max_hotel_id', '$city_id', '$hotel_name', '$mobile_no', '$landline_no', '$email_id', '$contact_person_name', '$immergency_contact_no', '$hotel_address', '$country', '$website', '$opening_balance','$rating_star','$meal_plan','$hotel_type', '$bank_name','$account_name','$account_no','$branch','$ifsc_code', '$service_tax_no', '$active_flag','$state','$side','$supp_pan','$as_of_date','$description','$policies','$amenities', '$cwb_from', '$cwb_to', '$cwob_from', '$cwob_to')");
...
function hotel_master_update( $hotel_id, $vendor_login_id, $city_id, $hotel_name, $mobile_no, $landline_no, $email_id, $contact_person_name, $immergency_contact_no, $hotel_address, $country, $website, $opening_balance,$rating_star, $active_flag, $bank_name,$account_name ,$account_no, $branch, $ifsc_code, $service_tax_no,$state,$side1,$supp_pan,$as_of_date,$description,$policies,$amenities,$hotel_type,$meal_plan,$cwb_from,$cwb_to,$cwob_from,$cwob_to)
{
  $city_id = mysql_real_escape_string($city_id);
  $mobile_no = mysql_real_escape_string($mobile_no);
  $email_id = mysql_real_escape_string($email_id);
