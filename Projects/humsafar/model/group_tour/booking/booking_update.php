...
public function complete_booking_information_update()
{
  ...
  $booking_save_transaction->finance_update($tourwise_id, $row_spec, $sq_date['form_date']);
...

 {
...

      {
...

        {
          ...
          $sq = mysql_query("update plane_master set date = '$plane_travel_date[$i]', from_location = '$plane_from_location[$i]', to_location = '$plane_to_location[$i]', company = '$plane_company[$i]', amount = '$plane_amount[$i]', seats = '$plane_seats[$i]', arraval_time = '$arravl_arr[$i]', from_city='$from_city_id_arr[$i]', to_city='$to_city_id_arr[$i]'  where plane_id = '$plane_id[$i]' and tourwise_traveler_id = '$tourwise_id'");
        ...
        {  
          ...
          $sq = mysql_query(" insert into plane_master (plane_id, tourwise_traveler_id, date, from_city, to_city, from_location, to_location, company, amount, seats, arraval_time ) values ('$max_plane_id', '$tourwise_id', '$plane_travel_date[$i]', '$from_city_id_arr[$i]', '$to_city_id_arr[$i]', '$plane_from_location[$i]', '$plane_to_location[$i]', '$plane_company[$i]', '$plane_amount[$i]', '$plane_seats[$i]', '$arravl_arr[$i]') ");
