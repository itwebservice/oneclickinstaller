	...
	while($row_bookings = mysql_fetch_assoc($sq_bookings)){
		...
		while($row_per = mysql_fetch_assoc($sq_personal_info)){
			...
			$sq_count = mysql_num_rows(mysql_query("SELECT * from  remainder_status where remainder_name = 'git_feedback_remainder' and date='$today' and status='Done'"));
				...
				feedback_email_send($email_id,$tourwise_traveler_id,$tour_name,$journey_date,$username,$password,$cust_name,$customer_id);
				}
			}
			if($mobile_no!=""){
...
function feedback_email_send($email_id,$tourwise_traveler_id,$tour_name,$journey_date,$username,$password,$cust_name,$customer_id)
{
				...
				<table style="width:100%">
					<tr>
						<td>
							...
							<p style="line-height: 24px;"><strong style="'.$mail_strong_style.'">Journey Date :</strong> '.$journey_date.'</p>
						</td>
					</tr>
					<tr>
						<td>
	...
	$model->app_email_send('79',$email_id, $content,$subject,'1');
}
