...
public function refund_mail_send($passport_id,$refund_amount,$refund_date,$refund_mode,$transaction_id){
      ...
      <table cellspacing="0" style="width:100%">
	      ...
	      <tr>
	        <td style="background: rgba(255, 255, 255, 0.23);color: #000;">
	          <p style="line-height: 24px;padding: 0 0 0 15px;">Booking ID : '.$sq_passport_info['passport_id'].'</p>
  ...
  global $model;
  $model->app_email_send('110',$cust_email['email_id'], $content, $subject);
}
...
public function finance_save($refund_id)
{
    ...
    $payment_particular = get_refund_paid_particular(get_passport_booking_id($passport_id,$yr2), $refund_date, $refund_amount, $refund_mode,get_passport_booking_refund_id($refund_id,$yr2));
    $ledger_particular = '';
    $gl_id = $pay_gl;
    $payment_side = "Credit";
    $clearance_status = "";
...
public function bank_cash_book_save($refund_id)
{
	...
	$transaction_id = $transaction_id;
	$bank_id = $bank_id;
	$particular = get_refund_paid_particular(get_passport_booking_id($passport_id,$yr2), $refund_date, $refund_amount, $refund_mode, get_passport_booking_refund_id($refund_id,$yr2));
