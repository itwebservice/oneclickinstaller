					...
					<tbody>
			        	...
			        	foreach($services_arr as $service){
			        		...
			        		$service_id = $service['value'];
									$sq_service = mysql_fetch_assoc(mysql_query("select * from itinerary_paid_services where service_id='$service_id'"));
									$sq_city = mysql_fetch_assoc(mysql_query("select * from city_master where city_id='$sq_service[city_id]'"));
			        		?>
							<tr>
								<td><?= $count ?></td>
								<td><?= $sq_city['city_name'].' : '.$sq_service['service_name'] ?></td>
							</tr>
			        		<?php
			        	}
			        	?>
