...
<table class="table" id="tbl_visa_list" style="margin: 20px 0 !important;">
	<thead>
	...
	<tbody>
		...
		while($row_visa = mysql_fetch_assoc($sq_visa)){
			...
			<tr class="<?= $bg ?>">
				...
				<td><?php echo get_date_user($row_visa['created_at']); ?></td>
				<td>
					<button class="btn btn-info btn-sm" onclick="visa_view_modal(<?= $row_visa['visa_id']?>)" title="View Detail"><i class="fa fa-eye" aria-hidden="true"></i></button>
				</td>	
				<td class="info text-right"><?= number_format($row_visa['visa_issue_amount'],2) ?></td>
				<td class="info text-right"><?= number_format($row_visa['service_charge'],2)?></td>
				<td class="info text-right"><?= number_format($row_visa['service_tax_subtotal'],2); ?></td>
				...
				<td>
					<button class="btn btn-info btn-sm" onclick="payment_view_modal(<?= $row_visa['visa_id'] ?>)" title="View Detail"><i class="fa fa-eye" aria-hidden="true"></i></button>
				</td>
				<td class="warning text-right" text-right><?= number_format($bal, 2); ?></td>
				<td><?php echo $due_date; ?></td>
				<td><?php echo number_format($total_purchase,2); ?></td>
				<td>
					<button class="btn btn-info btn-sm" onclick="supplier_view_modal(<?= $row_visa['visa_id'] ?>)" title="View Detail"><i class="fa fa-eye" aria-hidden="true"></i></button>
				</td>
	...
	<tfoot>
		<th colspan="10"></th>
		<th colspan="3" class="danger text-right"><?php echo "TOTAL CANCEL : ".number_format($cancel_total,2); ?></th>
		<th colspan="3" class="info text-right"><?php echo "TOTAL SALE : ".number_format($sale_total,2); ?></th>
		<th colspan="3" class="success text-right"><?php echo "TOTAL PAID : ".number_format($paid_total,2); ?></th>
		<th colspan="3" class="warning text-right"><?php echo "TOTAL BALANCE : ".number_format($balance_total,2); ?></th>		
		<th></th>
	</tfoot>
</table>
</div> </div> </div>
