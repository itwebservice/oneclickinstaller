...
<div id="div_train_ticket_customer_list_reflect" class="main_block"></div>
...
<script>
	...
	function calculate_total_amount()
	{
		...
		if(gst_on=="Service Charge"){ 
			service_tax_subtotal = (parseFloat(service_charge)/100)*parseFloat(service_tax);
		}
		if(gst_on=="Delivery Charges"){ 
			service_tax_subtotal = (parseFloat(delivery_charges)/100)*parseFloat(service_tax);
		}
		if(gst_on=="Service Charge+Delivery Charges"){ 
			var charge = parseFloat(service_charge) + parseFloat(delivery_charges);
			service_tax_subtotal = (parseFloat(charge)/100)*parseFloat(service_tax);
		}
		service_tax_subtotal = Math.round(service_tax_subtotal);
		$('#service_tax_subtotal').val(service_tax_subtotal.toFixed(2));

		net_total = parseFloat(basic_fair) + parseFloat(service_charge) + parseFloat(delivery_charges) + parseFloat(service_tax_subtotal);
