<tr>
    <td ><input id="chk_hotel<?= $prefix ?>_1" type="checkbox" checked ></td>
    <td><input maxlength="15" type="text" name="username"  value="1" placeholder="Sr. No." disabled/></td>
    <td style="width:104px"><select id="city_id<?= $prefix ?>1" name="city_id<?= $prefix ?>1" title="City" onchange="hotel_name_list_load(this.id)" class="app_select2" style="width:100%">
            <?php get_cities_dropdown(); ?>
        </select>
    </td>    
    <td style="width:130px"><select id="hotel_id1" name="hotel_id1" title="Hotel">
            <option value="">*Select Hotel</option>
        </select>
    </td>
    <td><input type="text" class="app_datetimepicker" id="check_in<?= $prefix ?>1" name="check_in<?= $prefix ?>1" placeholder="Check-In Date" title="Check-In Date" value="<?= date('d-m-Y H:i:s')?>" onchange="get_to_datetime(this.id,'check_out<?= $prefix ?>1')"></td>
    <td><input type="text" class="app_datetimepicker" id="check_out<?= $prefix ?>1" name="check_out<?= $prefix ?>1" placeholder="Check-Out Date" title="Check-Out Date" value="<?= date('d-m-Y H:i:s')?>"></td>
    <td><input type="text" id="no_of_nights<?= $prefix ?>1" name="no_of_nights<?= $prefix ?>1" placeholder="*No Of Nights" title="No Of Nights" onchange="validate_balance(this.id)"></td>
    <td><input type="text" id="rooms<?= $prefix ?>1" name="rooms<?= $prefix ?>1" placeholder="*No Of Rooms" title="No Of Rooms" onchange="validate_balance(this.id)"></td>
