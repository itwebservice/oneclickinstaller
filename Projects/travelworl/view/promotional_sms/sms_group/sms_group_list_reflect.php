...
<table id="tbl_sms_group" class="table table-bordered" style="margin: 20px 0 !important;">
	...
	<tbody>
		...
		while($row_sms_group = mysql_fetch_assoc($sq_sms_group)){
			...
			<tr>
				<td><?= ++$count ?></td>
				<td><?= $row_sms_group['sms_group_name'] ?></td>
				<td>
					<button class="btn btn-info btn-sm" onclick="sms_group_edit_modal(<?= $row_sms_group['sms_group_id'] ?>)" title="Edit Group"><i class="fa fa-pencil-square-o"></i></button>
				</td>
			</tr>
			<?php
		}
