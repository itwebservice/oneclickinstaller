	...
	<tbody>
		...
		while($row_customer = mysql_fetch_assoc($sq_customer)){
			...
			<tr class="<?= $bg ?>">
				...
				<td><?= $sq_country['country_name'].'('.$sq_country['country_code'].')' ?></td>
				<td>
				...
				<td>
					<button class="btn <?= $color ?> btn-sm" onclick="b2b_update_modal(<?= $row_customer['register_id'] ?>)" title="Update Customer"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></button>
				</td>
				<td>
