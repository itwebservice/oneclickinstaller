...
<table class="table table-bordered no-marg" id="exc_payment_list" style="margin: 20px 0 !important;">
	<thead>
			...
			<tr class="<?= $bg?>">
				...
				<td>
					...
					if($row_exc_payment['payment_mode']=="Cash" || $row_exc_payment['payment_mode']=="Cheque"){
						?>
						<input type="checkbox" id="chk_exc_payment_<?= $count ?>" name="chk_exc_payment" value="<?= $row_exc_payment['payment_id'] ?>">
						<?php	
					}
					?>					
				</td>
				...
				<td>
					<button class="btn btn-info btn-sm" onclick="exc_payment_update_modal(<?= $row_exc_payment['payment_id'] ?>)" title="Edit Detail"><i class="fa fa-pencil-square-o"></i></button>
				</td>
			</tr>
			<?php
			}
