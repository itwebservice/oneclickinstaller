...
<table class="table" id="table_hotel_inv" style="margin: 20px 0 !important;">
	...
	<tbody>
		...
		while($row_ser = mysql_fetch_assoc($sq_serv)){
			...
			$sq_exc = mysql_fetch_assoc(mysql_query("select service_id, service_name from itinerary_paid_services where service_id='$row_ser[exc_id]'"));
		...
		?>
			<tr class="<?= $bg ?>">
				...
				<td onclick="history_modal(<?= $row_ser['entry_id'] ?>)" style="color:#009898;"><?= $sq_exc['service_name'] ?></td>
				<td><?= $row_ser['rate'] ?></td>
