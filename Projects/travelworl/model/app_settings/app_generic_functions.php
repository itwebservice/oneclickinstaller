...
if (!function_exists('begin_panel')) {
  function begin_panel($title,$entry_id) {
    if($entry_id!=''){
      ...
      <div class="pull-right header_btn"><button title="Video" onclick="display_description(1,'.$entry_id.')"><a><i class="fa fa-play" aria-hidden="true" title="Video"></i></a></button></div>
...
function sundry_creditor_balance_update()
  {
    ...
    $sq_hotel_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from hotel_master"));
    $sq_tranport_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from transport_agency_master"));
    $sq_dmc_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from dmc_master"));
    $sq_car_rental_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from car_rental_vendor"));
    $sq_visa_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from visa_vendor"));
    $sq_passport_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from passport_vendor"));
    $sq_ticket_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from ticket_vendor"));
    $sq_train_ticket_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from train_ticket_vendor"));
    $sq_site_seeing_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from site_seeing_vendor"));
    $sq_insuarance_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from insuarance_vendor"));
    $sq_other_vendors_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from other_vendors"));
    $sq_cruise_balance = mysql_fetch_assoc(mysql_query("select sum(opening_balance) as opening_balance from cruise_master"));
