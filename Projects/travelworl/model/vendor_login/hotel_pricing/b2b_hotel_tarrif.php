...
public function hotel_tarrif_save(){
	...
	if($sq_login){
		...
		for($i=0; $i<sizeof($from_date_array); $i++){
			...
			$sq1 = mysql_query("INSERT INTO `hotel_offers_tarrif`(`entry_id`, `pricing_id`,`type`, `from_date`, `to_date`, `offer`,`offer_amount`, `agent_type`) VALUES ('$max_entry_id','$pricing_id','$type_array[$i]','$from_date_array[$i]','$to_date_array[$i]','$offer_array[$i]','$offer_amount_array[$i]','$agent_array[$i]')");
			if(!$sq1){
				$GLOBALS['flag'] = false;
