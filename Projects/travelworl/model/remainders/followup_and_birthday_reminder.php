 ...
 include_once('../model.php');
...

  {
	...
	while($row_enq = mysql_fetch_assoc($sq_enquiry)){
		...
		if($sq_enquiry_entry['followup_status']=="Active" && date('Y-m-d H:i', strtotime($sq_enquiry_entry['followup_date']))==$today){
			...
			<td>
				...
				<table style="background: #fff; color: #22262e; font-size: 13px;width:100%; margin-bottom:20px;">
					...
					<tr>
						...
						<th colspan="5" style="'.$td_style.'">Todays Followups</th>
					...
					<tr>
						...
						<th style=" padding-left: 10px;border: 1px solid #c1c1c1;text-align: left;font-weight: 500;background: #ddd;font-size: 14px;color: #22262E">Tour Name</th>
						...
						<th style=" padding-left: 10px;border: 1px solid #c1c1c1;text-align: left;font-weight: 500;background: #ddd;font-size: 14px;color: #22262E">User</th>
					...
					while($row_enq = mysql_fetch_assoc($sq_enquiry)){
						...
						if($enquiry_content_arr1['name']=="tour_name")

							{ $tour_name = $enquiry_content_arr1['value']; }
						...
						if($sq_enquiry_entry['followup_status']=="Active" && date('Y-m-d', strtotime($sq_enquiry_entry['followup_date']))==$today){
							...
							$content1 .= '
								...
								<tr>
									...
									<td style="color: #777;font-size: 14px;text-align: left;padding-left: 10px;font-weight: 500;">'.$row_enq['name'].'</td>

									<td style="color: #777;font-size: 14px;text-align: left;padding-left: 10px;font-weight: 500;">'.$tour_name.'</td>

									<td style="color: #777;font-size: 14px;text-align: left;padding-left: 10px;font-weight: 500;">'.$row_enq['mobile_no'].'</td>

									<td style="color: #777;font-size: 14px;text-align: left;padding-left: 10px;font-weight: 500;">'.$sq_emp['first_name'].' '.$sq_emp['last_name'].'</td>
	...
	$subject = 'Todays_ followup';
		...
		$model->app_email_send('69',$to, $content,$subject,'1');
	}
