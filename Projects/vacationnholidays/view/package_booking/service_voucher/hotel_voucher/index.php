...
?>
<?= begin_panel('Hotel Service Voucher',45) ?>
...
<script>
	function feedback_content_reflect()
	{
		...
		if(id=="rd_package_tour"){
			$.post('package_tour/package_booking_voucher.php', {branch_status : branch_status}, function(data){
				$('#div_feedback_mail_content').html(data);
			});
		}
		if(id=="rd_hotel_tour"){
			$.post('hotel_booking/hotel_booking_voucher .php', { branch_status : branch_status}, function(data){
				$('#div_feedback_mail_content').html(data);
			});
		}
	}
