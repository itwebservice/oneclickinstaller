...
<div id="div_modal"></div>
  ...
  <table id="tbl_email_id" class="table table-bordered" style="margin: 20px 0 !important;">
    <thead>
...
function email_list_reflect(limit, start){
  $('#tbl_email_id').show();
  var branch_status = $('#branch_status').val();

	$.ajax({
    url:"email_id/email_id_list_reflect.php",
    method:"POST",
    data:{branch_status : branch_status,limit:limit, start:start },
    cache:false,
    success:function(data){
      $('#tbl_email_id tbody').append(data);
        action = 'inactive';
    }
	});
}
...
if(action == 'inactive'){
	...
	action = 'active';
	email_list_reflect(limit, start);
}
$("div.app_content_wrap").on('scroll' , function(event){
if($('#tbl_email_id').is(":visible")==true){
	if($("div.app_content_wrap").scrollTop() + $("div.app_content_wrap").height() > $("#tbl_email_id tbody").height() && action == 'inactive'){
		action = 'active';
		start = start + limit;
		setTimeout(function(){
			email_list_reflect(limit, start);
...
function email_id_list_reflect(limit , start){
		window.limit=20;
		window.start=0;
		$('#tbl_email_id').show();
		$('#tbl_email_id tbody tr').remove();
		var branch_status = $('#branch_status').val();
		$.post('email_id/list_reflect.php', {limit : limit, start : start, branch_status : branch_status}, function(data){
			$('#tbl_email_id tbody').html(data);
		});
}
