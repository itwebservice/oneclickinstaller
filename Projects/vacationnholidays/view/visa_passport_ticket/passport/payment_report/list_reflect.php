...
<table class="table table-bordered" id="tbl_passport_list" style="margin: 20px 0 !important;">
	...
	<tbody>
		...
		while($row_passport = mysql_fetch_assoc($sq_passport)){
			...
			$url1 = BASE_URL."model/app_settings/print_html/invoice_html/body/passport_body_html.php?invoice_no=$invoice_no&invoice_date=$invoice_date&customer_id=$customer_id&service_name=$service_name&basic_cost=$basic_cost&taxation_type=$taxation_type&service_tax_per=$service_tax_per&service_tax=$service_tax&net_amount=$net_amount&service_charge=$service_charge&total_paid=$paid_amount&balance_amount=$bal_amount&sac_code=$sac_code&branch_status=$branch_status&passport_id=$passport_id";
			?>	
			<tr class="<?= $bg ?>">
				<td><?= ++$count ?></td>
				...
				<td><?php echo get_date_user($row_passport['created_at']); ?></td>
				<td>
					<button class="btn btn-info btn-sm" onclick="passport_view_modal(<?= $row_passport['passport_id'] ?>)" title="View Detail"><i class="fa fa-eye" aria-hidden="true"></i></button>
				</td>
				<td class="info text-right"><?= number_format($row_passport['passport_issue_amount'],2) ?></td>
				<td class="info text-right"><?= number_format($row_passport['service_charge'],2)?></td>
				<td class="info text-right"><?= number_format($row_passport['service_tax_subtotal'],2); ?></td>
				<td class="info text-right"><?= number_format($row_passport['passport_total_cost'],2) ?></td>
				...
				<td>
					<button class="btn btn-info btn-sm" onclick="payment_view_modal(<?= $row_passport['passport_id'] ?>)" title="View Detail"><i class="fa fa-eye" aria-hidden="true"></i></button>
				</td>
				<td class="warning text-right"><?= number_format($bal_amount, 2); ?></td>
				<td><?php echo ($row_passport['due_date'] == '1970-01-01') ? 'NA' : get_date_user($row_passport['due_date']); ?></td>
				<td><?php echo number_format($total_purchase,2); ?></td>
				<td>
					<button class="btn btn-info btn-sm" onclick="supplier_view_modal(<?= $row_passport['passport_id'] ?>)" title="View Detail"><i class="fa fa-eye" aria-hidden="true"></i></button>
				</td>
