...
<table class="table table-hover" id="tbl_payment_list" style="margin: 20px 0 !important;">
				...
				<tr class="<?= $bg?>">				
					...
					<td>
						...
						if($row_train_ticket_payment['payment_mode']=="Cash" || $row_train_ticket_payment['payment_mode']=="Cheque"){
							?>
							<input type="checkbox" id="chk_train_ticket_payment_<?= $count ?>" name="chk_train_ticket_payment" value="<?= $row_train_ticket_payment['payment_id'] ?>">
							<?php	
						}
						?>					
					</td>
					<td><?= get_train_ticket_booking_id($row_train_ticket_payment['train_ticket_id'],$year) ?></td>
					<td><?= $customer_name ?></td>
					...
					<td>
						<button class="btn btn-info btn-sm" onclick="train_ticket_payment_update_modal(<?= $row_train_ticket_payment['payment_id'] ?>)" title="Edit Payment"><i class="fa fa-pencil-square-o"></i></button>
					</td>
				</tr>
				<?php
			}
	...
	<tfoot>
		<tr class="active">
			...
			<th colspan="3" class="warning text-right">Pending Clearance: <?= number_format($sq_pending_amount, 2); ?></th>
