...
function package_tour_booking_master_update()
{
    ...
    $transport_to_date = $_POST['transport_to_date'];
    ...
    if(!$sq_booking){
      ...
      $this->package_tour_tranpsort_information_save($booking_id, $transp_vehicle_arr, $transp_start_date, $trans_end_date,$trans_entry_id_arr);

      //** This function stores the excursion information
      $this->package_tour_exc_information_save($booking_id, $exc_city_arr, $exc_name_arr,$exc_entry_id_arr);
      ...
      $booking_update_transaction->finance_update($booking_id, $row_spec);
...
//Transport Save
function package_tour_tranpsort_information_save($booking_id, $transp_vehicle_arr, $transp_start_date, $trans_end_date,$trans_entry_id_arr){
    for($i=0; $i<sizeof($transp_vehicle_arr); $i++){
      ...
      if($trans_entry_id_arr[$i]!=''){
          $sq = mysql_query("update package_tour_transport_master set  transport_bus_id = '$transp_vehicle_arr[$i]', transport_from_date = '$transp_start_date[$i]', transport_to_date = '$trans_end_date[$i]' where entry_id='$trans_entry_id_arr[$i]'");
        ...
        $q1 = "insert into package_tour_transport_master (entry_id, booking_id, transport_bus_id, transport_from_date, transport_to_date) values ('$max_entry_id', '$booking_id', '$transp_vehicle_arr[$i]', '$transp_start_date[$i]', '$trans_end_date[$i]')";
        $sq = mysql_query($q1);
...
//Excursion Save
function package_tour_exc_information_save($booking_id, $exc_city_arr, $exc_name_arr,$exc_entry_id_arr){
  for($i=0; $i<sizeof($exc_city_arr); $i++){
    ...
    if($exc_entry_id_arr[$i]!=''){
        $sq = mysql_query("update package_tour_excursion_master set  city_id = '$exc_city_arr[$i]', exc_id = '$exc_name_arr[$i]' where entry_id='$exc_entry_id_arr[$i]'");
      ...
      $sq = mysql_query("insert into package_tour_excursion_master (entry_id, booking_id, city_id, exc_id) values ('$max_entry_id', '$booking_id', '$exc_city_arr[$i]', '$exc_name_arr[$i]')");
 ...
 function package_tour_traveling_information_update( $booking_id, $train_travel_date, $train_from_location, $train_to_location, $train_train_no, $train_travel_class, $train_travel_priority, $train_amount, $train_seats, $train_id, $plane_travel_date, $from_city_id_arr, $plane_from_location, $to_city_id_arr, $plane_to_location, $plane_amount, $plane_seats, $plane_company, $plane_id, $arrval_arr, $cruise_dept_date_arr, $cruise_arrival_date_arr, $route_arr, $cabin_arr, $sharing_arr, $cruise_seats_arr, $cruise_amount_arr,$cruise_id_arr)
 {
      ...
      for($i=0; $i<sizeof($plane_travel_date); $i++)
      {
        ...
        if($count_plane_details == 1)
        {
          ...
          $sq = mysql_query("update package_plane_master set date = '$plane_travel_date[$i]', from_location = '$plane_from_location[$i]', to_location = '$plane_to_location[$i]', company = '$plane_company[$i]', amount = '$plane_amount[$i]', seats = '$plane_seats[$i]', arraval_time = '$arrval_arr[$i]', from_city='$from_city_id_arr[$i]', to_city='$to_city_id_arr[$i]'  where plane_id = '$plane_id[$i]' and booking_id = '$booking_id'");
           if(!$sq)
           {
              $GLOBALS['flag'] = false;
