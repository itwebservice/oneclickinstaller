...
<table class="table" id="tbl_exc_list" style="margin: 20px 0 !important;">
	<thead>
	...
	<tbody>
		...
		while($row_exc = mysql_fetch_assoc($sq_exc)){
			...
			<tr class="<?= $bg ?>">
				...
				<td><?php echo get_date_user($row_exc['created_at']); ?></td>
				<td>
					<button class="btn btn-info btn-sm" onclick="exc_view_modal(<?= $row_exc['exc_id'] ?>)" title="View Detail"><i class="fa fa-eye" aria-hidden="true"></i></button>
				</td>	
				<td class="info text-right"><?= number_format($row_exc['exc_issue_amount'],2) ?></td>
				<td class="info text-right"><?= number_format($row_exc['service_charge'],2)?></td>
				<td class="info text-right"><?= number_format($row_exc['service_tax_subtotal'],2); ?></td>
				...
				<td>
					<button class="btn btn-info btn-sm" onclick="payment_view_modal(<?= $row_exc['exc_id'] ?>)" title="View Detail"><i class="fa fa-eye" aria-hidden="true"></i></button>
				</td>
				<td class="warning text-right" text-right><?= number_format($bal, 2); ?></td>
				<td><?php echo $due_date; ?></td>
				<td><?php echo number_format($total_purchase,2); ?></td>
				<td>
					<button class="btn btn-info btn-sm" onclick="supplier_view_modal(<?= $row_exc['exc_id'] ?>)" title="View Detail"><i class="fa fa-eye" aria-hidden="true"></i></button>
				</td>
