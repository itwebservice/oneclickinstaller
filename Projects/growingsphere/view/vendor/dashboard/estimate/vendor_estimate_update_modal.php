    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
			...
			<legend>Supplier Type</legend>
				...
				<div class="row">
					<div class="col-md-4 col-sm-6 col-xs-12 mg_bt_10">
						<select name="vendor_type1" id="vendor_type1" title="Vendor Type" onchange="vendor_type_data_load(this.value, 'div_vendor_type_content1', '1')" disabled>
					...
					<script>
						vendor_type_data_load('<?= $sq_vendor_estimate['vendor_type'] ?>', 'div_vendor_type_content1', '1', <?= $sq_vendor_estimate['vendor_type_id'] ?>);
					</script>
				</div>
			...
			<legend>Supplier Purchase</legend>
				...
				<div class="row">					
					<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
						<input type="text" id="basic_cost" name="basic_cost" placeholder="Basic Cost" title="Basic Cost" onchange="validate_balance(this.id);calculate_estimate_amount('')" value="<?= $sq_vendor_estimate['basic_cost'] ?>">
					</div>
					<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
					...
					<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
						<input type="text" id="service_charge" name="service_charge" placeholder="Supplier Service Charge" title="Supplier Service Charge" onchange="validate_balance(this.id);calculate_estimate_amount('')" value="<?= $sq_vendor_estimate['service_charge'] ?>">
					</div>
					<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
					...
					<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
			          <select name="taxation_type" id="taxation_type" title="Taxation Type">
			            <?php if($sq_vendor_estimate['taxation_type'] != ''){?>
			            <option value="<?= $sq_vendor_estimate['taxation_type'] ?>"><?= $sq_vendor_estimate['taxation_type'] ?></option>
						<?php } 
			            get_taxation_type_dropdown($setup_country_id) ?>
			          </select>
			        </div>
					<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
						<select name="taxation_id" id="taxation_id" title="Tax" onchange="generic_tax_reflect(this.id, 'service_tax', 'calculate_estimate_amount', '');">		
							<?php 
		                       if($sq_vendor_estimate['taxation_id']!="" || $sq_vendor_estimate['taxation_id']!=0){
		                         $sq_taxation = mysql_fetch_assoc(mysql_query("select * from taxation_master where taxation_id='$sq_vendor_estimate[taxation_id]'"));
		                         $sq_tax_type = mysql_fetch_assoc(mysql_query("select * from tax_type_master where tax_type_id='$sq_taxation[tax_type_id]'"));
		                       }
	                         ?>
	                         <?php if($sq_taxation['taxation_id']!="" || $sq_taxation['taxation_id']!=0){?>
		                     	<option value="<?= $sq_taxation['taxation_id'] ?>"><?= $sq_tax_type['tax_type'].'-'.$sq_taxation['tax_in_percentage'] ?></option>
		                     <?php } ?>
							<?php get_taxation_dropdown(); ?>
				        </select>
				        <input type="hidden" id="service_tax" name="service_tax" value="<?= $sq_vendor_estimate['service_tax'] ?>">
					</div>			
					<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
					...
					<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
						<input type="text" id="discount" name="discount" placeholder="Discount" title="Discount" onchange="validate_balance(this.id);calculate_estimate_amount('')" value="<?= $sq_vendor_estimate['discount'] ?>">
					</div>
					<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
						<input type="text" id="our_commission" name="our_commission" placeholder="Our commission" title="Our commission" onchange="validate_balance(this.id);calculate_estimate_amount('')" value="<?= $sq_vendor_estimate['our_commission'] ?>">
					</div>
					<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
				...
				<div class="row">
					...
					<div class="col-md-2 col-sm-4 col-xs-12 mg_bt_10">
						<input type="text" placeholder="Purchase date" title="Purchase Date" id="purchase_date1" name="purchase_date1"  value="<?= date('d-m-Y', strtotime($sq_vendor_estimate['purchase_date'])) ?>" onchange="check_valid_date(this.id)">
					</div>
				</div>
			</div>
				...
				$.post(base_url+'view/load_data/finance_date_validation.php', { check_date: purchase_date }, function(data){
					if(data !== 'valid'){
						...
						$.ajax({
							...
							data:{ estimate_id : estimate_id, estimate_type : estimate_type, vendor_type : vendor_type, estimate_type_id : estimate_type_id, vendor_type_id : vendor_type_id, basic_cost : basic_cost, non_recoverable_taxes : non_recoverable_taxes, service_charge : service_charge, other_charges : other_charges, taxation_id : taxation_id, taxation_type : taxation_type, service_tax : service_tax, service_tax_subtotal : service_tax_subtotal, discount : discount, our_commission : our_commission, tds : tds, net_total : net_total, remark : remark, invoice_id : invoice_id, payment_due_date : payment_due_date,invoice_url : invoice_url,purchase_date : purchase_date },
