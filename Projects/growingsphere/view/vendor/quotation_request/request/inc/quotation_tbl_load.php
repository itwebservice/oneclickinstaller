...
if($quotation_for=="DMC"){
	...
	<div class="row mg_bt_20">
		<div class="col-md-12 text-right">
			<button type="button" class="btn btn-info btn-sm ico_left" onClick="addRow('tbl_vendor_quotation_dmc_entries')"><i class="fa fa-plus"></i>&nbsp;&nbsp;Add</button>
			<?php if($request_id==""){ ?>
		...
		<table class="table" id="tbl_vendor_quotation_dmc_entries">
			...
			if($request_id=="" || sizeof($dmc_entries)==0){
				...
				<tr style="height: 60px">
					...
					<td><input type="text" id="checkin_date" name="checkin_date" onchange="get_to_date(this.id,'checkout_date');validate_validDate('checkin_date' , 'checkout_date')" placeholder="Check-In Date" title="Check-In Date" class="app_datepicker"  value="<?= date('d-m-Y') ?>"></td>
					<td><input type="text" id="checkout_date" onchange="validate_issueDate('checkin_date' , 'checkout_date')"  name="checkout_date" placeholder="Check-Out Date" title="Check-Out Date" class="app_datepicker"  value="<?= date('d-m-Y') ?>"></td>
