	...
	while($row_entry = mysql_fetch_assoc($sq_entry)){
		...
		?>
		 <tr class="<?= $bg ?>">
		    ...
		    <td><input type="text" id="birth_date<?= $count ?>_u" name="birth_date" placeholder="Birth Date" title="Birth Date" class="app_datepicker" onchange="adolescence_reflect(this.id)" value="<?= get_date_user($row_entry['birth_date']) ?>"/></td>
