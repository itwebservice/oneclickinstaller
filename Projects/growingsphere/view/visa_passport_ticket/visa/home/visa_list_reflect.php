...
<table class="table table-bordered" id="tbl_visa_list" style="margin: 20px 0 !important;">
	...
	<tbody>
		...
		$sq_visa = mysql_query($query);		
			...
			<tr class="<?= $bg ?>">
				<td><?= ++$count ?></td>
				<td><?= get_visa_booking_id($row_visa['visa_id'],$yr) ?></td>
				<td><?= $customer_name ?></td>
				...
				<td>
					<button class="btn btn-info btn-sm" onclick="visa_update_modal(<?= $row_visa['visa_id'] ?>)" title="Edit Details"><i class="fa fa-pencil-square-o"></i></button>
				</td>
				<td><?= $emp_name ?></td>
			</tr>
			<?php
