        ...
        <table id="tbl_dynamic_cruise_quotation_update" name="tbl_dynamic_cruise_quotation_update" class="table mg_bt_0 table-bordered mg_bt_10">
        	...
        	if($sq_cruise_count==0){
				...
				<tr>
		            ...
		            <td><input type="text" id="cruise_departure_date" name="cruise_departure_date" placeholder="Departure Date Time" title="Departure Date Time" class="app_datetimepicker" value="<?= date('d-m-Y H:i:s') ?>" onchange="get_to_datetime(this.id,'cruise_arrival_date')"></td>
        		...
        		while($row_q_cruise = mysql_fetch_assoc($sq_q_cruise))
        		{
					...
					<tr>
			            ...
			            <td><input type="text" id="cruise_departure_date<?= $count ?>_u" name="cruise_departure_date<?= $count ?>_u" placeholder="Departure Date Time" title="Departure Date Time" class="app_datetimepicker" value="<?= get_datetime_user($row_q_cruise['dept_datetime']) ?>" onchange="get_to_datetime(this.id,'cruise_arrival_date<?= $count ?>_u')"></td>
			            <td><input type="text" id="cruise_arrival_date<?= $count ?>_u" name="cruise_arrival_date<?= $count ?>_u" placeholder="Arrival Date Time" title="Arrival Date Time" class="app_datetimepicker" value="<?= get_datetime_user($row_q_cruise['arrival_datetime']) ?>"></td>
